...
class Storage {
  ...
  public getItem = <T>(key: string): T | null => {
    ...
    try {
      ...
      console.error(`Failed to read from local/session storage - key ${key} – ${(err as Error).message}`)
    }
  ...
  public setItem = <T>(key: string, item: T): void => {
    ...
    try {
      ...
      console.error(`Failed to write to local/session storage - key ${key} – ${(err as Error).message}`)
    }
  }
  ...
  public removeItem = (key: string): void => {
    ...
    try {
      ...
      console.error(`Failed to remove from local/session storage - key ${key} – ${(err as Error).message}`)
    }
  }
}
